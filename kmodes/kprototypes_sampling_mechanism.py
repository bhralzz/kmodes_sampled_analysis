# -*- coding: utf-8 -*-
"""
Created on Wed Jul  3 11:24:29 2024
...
class kprototype_sampled_analysis(KPrototypes):
    def __init__(self, *args, **kwargs):
        # Initialize the base class with all arguments except the extra one
        super().__init__(*args, **kwargs)
        # self.sampled_df = self.df.sample(n=sampling_rows, random_state=42)  # random_state for reproducibility
        self.sampling_rows_count= 1_00
        pass

    # def analysis(self,desired_cluster_nu=10):
    #     kproto_model = KPrototypes(n_clusters=10, n_jobs=-1, random_state=42)
    #     clusters = kproto_model.fit_predict(
    #         self.sampled_df.values, categorical=[0,1,2,3]
    #     )

    def select_random_numbers(self,i, k, random_state=None):
        if random_state is not None:
            random.seed(random_state)
        random_numbers = random.sample(range(i), k)
    ...
    def fit_predict(self, X, y=None, **kwargs):
        ...
        random_indices = self.select_random_numbers(X.shape[0], self.sampling_rows_count, random_state=42)
        selected_array = X[random_indices]
        return self.fit(selected_array, **kwargs).predict(X, **kwargs)
